...
class Module extends \yii\base\Module
{
    ...
    public $allowedFileExtensions = ['jpg', 'jpeg', 'gif', 'png', 'webp'];
    ...
    public function init()
    {
        ...
        $this->components['assetManager']['bundles']['kartik\form\ActiveFormAsset'] = [
            'bsDependencyEnabled' => false,
        ];

        $this->params['bsDependencyEnabled'] = false;
        ...
        //set asset path
        $this->assetPublishedUrl =
            (new AssetManager)->getPublishedUrl("@vendor/noam148/yii2-image-manager/assets/source");
    ...
    private function _checkVariableConfiguration()
    {
        ...
        if (!is_bool($this->setBlameableBehavior)) {
            throw new InvalidConfigException(
                '$setBlameableBehavior only supports a boolean value, if you have a custom function make sure that you return a boolean.'
            );
        }
        // Check if the deleteOriginalAfterEdit is boolean
        if (!is_bool($this->deleteOriginalAfterEdit)) {
            throw new InvalidConfigException(
                '$deleteOriginalAfterEdit only supports boolean value, if you have a custom function make sure that your return a boolean.'
            );
        }
        ...
        if ($this->setBlameableBehavior) {
            ...
            $mRecordMigrationRun = Yii::$app->db
                ->createCommand(
